...
Only the portable (non-TLX, SM80+/CDNA) fused SwiGLU forward kernel is ported:
...
The upstream Blackwell (SM100+) TLX/TMA warp-specialized persistent path
(`_swiglu_fwd_tma_ws_persistent`, requiring `triton.language.extra.tlx` and
`triton.tools.tensor_descriptor`) and the `triton_swiglu` dispatcher that selects
it are dropped: on AMD/CDNA `is_sm100_plus()` is False so the portable path is the
one that runs. The `generative_recommenders.common.triton_autotune` dep is inlined.
"""
...
# =============================================================================
# Standard fused SwiGLU kernel for A100/H100/MI300X (non-TLX path).
#
# Fuses silu(x @ W_gate^T) * (x @ W_up^T) into a single kernel launch.
# Uses standard Triton pointer arithmetic (no TMA), works on SM80+ / CDNA.
#

...
`switch_to_contiguous_if_needed`, `is_sm90`, `is_sm100_plus`) are inlined below.
The CUDA fb path (`torch.ops.load_library(...)` +
`CUDA_JAGGED_DENSE_BMM_*` branches) is removed and the portable `@triton.jit`
path is kept; the SM checks are inlined to return False on non-NVIDIA so this
AMD/CDNA (gfx950) build always takes the portable Triton path. The autograd
...
# --- inlined NVIDIA SM checks (generative_recommenders.ops.utils) ---
# On AMD/CDNA these are False, which is why the host path below always selects
# the portable Triton kernel rather than the (removed) CUDA fb kernel.
def is_sm90() -> bool:
    return False
